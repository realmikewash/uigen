...
test("MessageList renders nothing when there are no messages", () => {
...
test("MessageList applies correct styling for user vs assistant messages", () => {
  ...
  const userMessage = screen.getByText("User message").closest(".rounded-lg");
  const assistantMessage = screen
    .getByText("Assistant message")
    .closest(".rounded-lg");

  // User messages should have blue background
  expect(userMessage?.className).toContain("bg-blue-500");
  expect(userMessage?.className).toContain("text-white");

  // Assistant messages should have gray background
  expect(assistantMessage?.className).toContain("bg-gray-50");
  expect(assistantMessage?.className).toContain("text-gray-900");
});
...
test("MessageList shows loading for assistant message with empty parts", () => {
  ...
  const loadingText = container.querySelectorAll(".text-gray-500");
